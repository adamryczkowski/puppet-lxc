		...
		if [[ ! $line =~ $regex1 ]]; then
			...
			if [[ "$line" =~ $regex1 ]]; then
				...
				if [[ "$curuser" == "$user" ]]; then
					...
					foundidcnt=$curidcnt
					if [ "$curbaseid" == "$userbaseid" ] || [ -z "$userbaseid" ]; then
						if [ "$curidcnt" == "$useridcnt" ] || [ -z "$useridcnt" ]; then
							fullmatch=1
						fi
					fi
				fi
...
case $action in
	...
	if [ "$mode" == "subgid" ]; then
		sudo usermod --add-subgids $userbaseid-$((useridcnt+userbaseid-1)) $user
	else
		sudo usermod --add-subuids $userbaseid-$((useridcnt+userbaseid-1)) $user
	fi
	echo "$@" > /var/log/max-subuid.log
	;;
	show)
	removeall=0
	finduserid
	...
	if [ "$foundcnt" -gt 0 ]; then
		...
		if [ "$fullmatch" == "1" ]; then
			...
			#multiple records exists but no one is good or mismatch
			exit 2 
		fi
	else
		#Nic nie znaleziono
